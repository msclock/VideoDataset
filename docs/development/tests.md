...
2. Align test modules with the modules to be tested.
   ...
   For example, tests for the `fast_context_queue.cli` module should be located in the file `tests/cli_test.py`. If there are too many test cases, they can be split into files within the `tests/cli/` directory, using a prefix for each test file.
